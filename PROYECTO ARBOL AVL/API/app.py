...
def ruta(x, y):
    ...
    return jsonify({"ruta": path}) if path else jsonify({"error": "No se pudo encontrar una ruta."}), 404
...
def asignar_espacio():
    ...
    return jsonify({"espacio_asignado": espacio_asignado}) if espacio_asignado else jsonify({"error": "No hay espacios libres."}), 404
